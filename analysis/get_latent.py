...
SaveFolderPath = f'models/TotalDeltaETM_allgenes_ep{EPOCH}_nlv{nlv}_bs{bs}_combineby{combine_method}'
...
rho_df = pd.DataFrame(model.module.decoder.rho.detach().cpu().numpy(), index = ['topic_' + str(j) for j in range(topics_np.shape[1])], columns = model.adata.var.index).T
rho_df.to_csv(os.path.join(SaveFolderPath,"rho_weights.csv"))
delta_df = pd.DataFrame(model.module.decoder.delta.detach().cpu().numpy(), index = ['topic_' + str(j) for j in range(topics_np.shape[1])], columns = model.adata.var.index).T
